...
export function generateMix(params: MixGenerationParams): MixResult {
  ...
  // Convert targetDuration from minutes to seconds for comparison
  const targetDurationSeconds = targetDuration * 60;

  // Generate rest of mix
  while (availableTracks.length > 0) {
    // Check if mix is complete based on duration type
    if (durationType === 'time' && totalDuration >= targetDurationSeconds) break;
    if (durationType === 'tracks' && mixTracks.length >= targetTracks) break;

    // Calculate position based on duration type
    const position = durationType === 'time' 
      ? Math.min(totalDuration / targetDurationSeconds, 1.0)
      : Math.min((mixTracks.length - 1) / (targetTracks - 1), 1.0);
